...
tags: []
draft: no
quiz: https://rstudio.cloud/spaces/170089/project/2982800
---
...
We're going to use [hourly recorded surface data](C282_hourly_data.csv) from the same cruise we have looked at previously from Woods Hole, MA to Grenada. The columns we are going to focus on are:
...
Could certainly be seen as much easier. The only issue I've come across with `cowplot` is that it treats each plot as independent so if there is some difference in, for example, the plot placement of the y axis then they figures wont quite be aligned. It may be that this is not an issue for your plots.
